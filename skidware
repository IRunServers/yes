...
local SkidWareFolder = Instance.new("Folder", workspace)
SkidWareFolder.Name = "SkidWare-Folder"
...
LegitSection:AddToggle('Enabled', false, function(Boolean)
    SkidWareSettings.Aiming.Aimbot.Enabled = Boolean
end)

local AimAssistToggle = LegitSection:AddToggle('Aim Assist', false, function(Boolean)
    SkidWareSettings.Aiming.Aimbot.AimAssist = Boolean
end)

AimAssistToggle:AddKeybind()

local AimbotFOVToggle = LegitSection:AddToggle('Show FOV', false, function(Boolean)
    SkidWareSettings.Aiming.Aimbot.ShowFOV = Boolean
end)

AimbotFOVToggle:AddColorpicker(SkidWareSettings.Aiming.Aimbot.FOVColor, function(Color)
    SkidWareSettings.Aiming.Aimbot.FOVColor = Color
end)

LegitSection:AddToggle('Use Nearest Distance', false, function(Boolean)
    SkidWareSettings.Aiming.Aimbot.UseNearestDistance = Boolean
end)

LegitSection:AddToggle('Wall Check', false, function(Boolean)
    SkidWareSettings.Aiming.Aimbot.WallCheck = Boolean
end)

LegitSection:AddToggle('Knock Out Check', false, function(Boolean)
    SkidWareSettings.Aiming.Aimbot.KnockedOutCheck = Boolean
end)

LegitSection:AddToggle('Grabbed Check', false, function(Boolean)
    SkidWareSettings.Aiming.Aimbot.GrabbedCheck = Boolean
end)

LegitSection:AddDropdown('Hitboxes', {"Head", "HumanoidRootPart"}, {"Head"}, true, function(Value)
    SkidWareSettings.Aiming.Aimbot.Hitboxes = Value
end)
...
local TargetAimToggle = RageSection:AddToggle('Target Aim Enabled', false, function(Boolean)
    SkidWareSettings.Aiming.TargetAim.Enabled = Boolean
end)

TargetAimToggle:AddKeybind()

local SilentAimFOVToggle = RageSection:AddToggle('Show FOV', false, function(Boolean)
    SkidWareSettings.Aiming.TargetAim.ShowFOV = Boolean
end)

SilentAimFOVToggle:AddColorpicker(Color3.new(0.603921, 0.011764, 1), function(Color)
    SkidWareSettings.Aiming.TargetAim.FOVColor = Color
end)

RageSection:AddToggle('Wall Check', false, function(Boolean)
    SkidWareSettings.Aiming.TargetAim.WallCheck = Boolean
end)

RageSection:AddToggle('Knock Out Check', false, function(Boolean)
    SkidWareSettings.Aiming.TargetAim.KnockedOutCheck = Boolean
end)

RageSection:AddToggle('Grabbed Check', false, function(Boolean)
    SkidWareSettings.Aiming.TargetAim.GrabbedCheck = Boolean
end)

RageSection:AddDropdown('Hitboxes', {"Head", "HumanoidRootPart"}, {"Head"}, true, function(Value)
    SkidWareSettings.Aiming.TargetAim.Hitboxes = Value
end)
...
AimingSettings:AddToggle('Ping Based Prediction', false, function(Boolean)
    SkidWareSettings.Aiming.AimingSettings.PingBasedPrediction = Boolean
end)

AimingSettings:AddDropdown('Aim Assist Type', {"Camera", "Mouse"}, "Camera", false, function(Option)
    SkidWareSettings.Aiming.AimingSettings.AimAssistType = Option
end)

local SmoothingTracing = AimingSettings:AddToggle('Smoothing Tracing', false, function(Boolean)
    SkidWareSettings.Aiming.AimingSettings.SmoothingTracing = Boolean
end)

SmoothingTracing:AddSlider(2, 5, 10, 1, function(Value)
    SkidWareSettings.Aiming.AimingSettings.SmoothingTracingAmount = Value
end)

local MovementPredToggle2 = AimingSettings:AddToggle('Custom Prediction', false, function(Boolean)
    SkidWareSettings.Aiming.AimingSettings.MovementPrediction = Boolean
end)

MovementPredToggle2:AddSlider(1, 1, 5, 1, function(Value)
    SkidWareSettings.Aiming.AimingSettings.MovementPredictionAmount = 0.165 * tonumber("1." ..Value)
end)

local HitChanceToggle2 = AimingSettings:AddToggle('Hit Chance', false, function(Boolean)
    SkidWareSettings.Aiming.AimingSettings.HitChance = Boolean
end)

HitChanceToggle2:AddSlider(0, 100, 100, 1, function(Value)
    SkidWareSettings.Aiming.AimingSettings.HitChanceAmount.HitPercent = tonumber(Value)
    SkidWareSettings.Aiming.AimingSettings.HitChanceAmount.NotHitPercent = tonumber(100 - SkidWareSettings.Aiming.AimingSettings.HitChanceAmount.HitPercent)
end)
...
local TargetBind = TargetAimSettings:AddKeybind("Bind", false, function()
    ...
    if SkidWareSettings.Aiming.TargetAim.Enabled then
        ...
        if NearestTarget and Visible(NearestTarget.Character.HumanoidRootPart, LocalPlayer.Character.HumanoidRootPart) then
            SkidWareSettings.Aiming.TargetAim.Target = NearestTarget.Name
            if SkidWareSettings.Aiming.TargetAimSettings.NotificationAlert then
                Notify({
                    Title = "skidware",
                    Description = "the skid that was locked onto was named: " .. NearestTarget.Name,
                    Duration = 3
                })
                Notify({
                    Title = "skidware",
                    Description = "display name: " .. NearestTarget.DisplayName,
                    Duration = 3
                })
                Notify({
                    Title = "skidware",
                    Description = "they are: " .. NearestTarget.AccountAge .. " days old",
                    Duration = 3
                })
            end
        end
...
TargetAimSettings:AddToggle('Unlock Target Knocked', false, function(State)
    SkidWareSettings.Aiming.TargetAimSettings.UnlockTargetKnocked = State
end)

TargetAimSettings:AddToggle('Notification Alert', false, function(State)
    SkidWareSettings.Aiming.TargetAimSettings.NotificationAlert = State
end)
...
WhitelistSection:AddToggle('Whitelist Enabled', false, function(State)
    SkidWareSettings.Aiming.Whitelist.Enabled = State
end)
...
WhitelistSection:AddTextbox("Username", nil, function(Text)
    if Text ~= nil and Find(Text) ~= nil and Players:FindFirstChild(Find(Text)) then
        SkidWareSettings.Aiming.Whitelist.Holder = Find(Text)
    else
        Notify({
            Title = "SkidWare",
...
WhitelistSection:AddButton('Add', function(State)
    if SkidWareSettings.Aiming.Whitelist.Holder ~= nil and Players:FindFirstChild(SkidWareSettings.Aiming.Whitelist.Holder) then
        if table.find(SkidWareSettings.Aiming.Whitelist.Players, SkidWareSettings.Aiming.Whitelist.Holder) then
            Notify({
                Title = "SkidWare",
                Description = SkidWareSettings.Aiming.Whitelist.Holder .. " is whitelisted.",
                Duration = 3
            })
        else
            Insert(SkidWareSettings.Aiming.Whitelist.Players, SkidWareSettings.Aiming.Whitelist.Holder)
            Notify({
                Title = "SkidWare",
                Description = "Whitelisted " .. SkidWareSettings.Aiming.Whitelist.Holder,
                Duration = 3
            })
        end
    else
...
WhitelistSection:AddButton('Remove', function()
    if SkidWareSettings.Aiming.Whitelist.Holder ~= nil and Players:FindFirstChild(SkidWareSettings.Aiming.Whitelist.Holder) then
        if table.find(SkidWareSettings.Aiming.Whitelist.Players, SkidWareSettings.Aiming.Whitelist.Holder) then
            Remove(SkidWareSettings.Aiming.Whitelist.Players, SkidWareSettings.Aiming.Whitelist.Holder)
            Notify({
                Title = "SkidWare",
                Description = "Removed " .. SkidWareSettings.Aiming.Whitelist.Holder,
                Duration = 5
            })
        else
            Notify({
                Title = "SkidWare",
                Description = SkidWareSettings.Aiming.Whitelist.Holder .. " is not whitelisted.",
                Duration = 5
            })
        end
    else
...
WhitelistSection:AddToggle('Crew Whitelist', false, function(State)
    SkidWareSettings.Aiming.Whitelist.CrewEnabled = State
end)

WhitelistSection:AddToggle('Friends Whitelist', false, function(State)
    SkidWareSettings.Aiming.Whitelist.FriendsWhitelist = State
end)
...
local UndergroundWallBangToggle = BlatantAntiAimSector:AddToggle('Underground Wallbang', false, function(State)
    pcall(function()
        if State then
			...
			SkidWareSettings.Blatant.BlatantAA.UndergroundWallbang = true
            SpeedToggle:Set(true)
		else
			LocalPlayer.Character.HumanoidRootPart.CFrame = LocalPlayer.Character.HumanoidRootPart.CFrame * CFrame.new(0, 9.5, 0)
			Cham(LocalPlayer, false)
			Float:Destroy()
			SkidWareSettings.Blatant.BlatantAA.UndergroundWallbang = false
...
local LegitAntiAimToggle = LegitAntiAimSector:AddToggle('Legit AA Enabled', false, function(State)
    SkidWareSettings.Blatant.LegitAA.Enabled = Value
end)

LegitAntiAimSector:AddToggle('Anti Aim At', false, function(State)
    SkidWareSettings.Blatant.LegitAA.AntiAimAt = State
end)

LegitAntiAimSector:AddSlider("Anti Aim At Distance", 2.5, 20, 100, 1, function(Value)
    SkidWareSettings.Blatant.LegitAA.AntiAimAtDistance = Value
end)
...
local FakeLagToggle = FakeLagSector:AddToggle('Fake Lag Enabled', false, function(State)
    SkidWareSettings.Blatant.FakeLag.Enabled = State
end)

FakeLagToggle:AddKeybind()

FakeLagToggle:AddSlider(1, 5, 10, 1, function(Value)
    SkidWareSettings.Blatant.FakeLag.TriggerAmount = Value
end)
...
GodModeSector:AddButton("Gun Only", function()
    SkidWareModule.God.GodBullet = true
    NilBody()
end)

GodModeSector:AddButton("Melee Only", function()
    SkidWareModule.God.GodMelee = true
    NilBody()
end)

GodModeSector:AddButton("Anti Ragdoll", function()
    SkidWareModule.God.AntiRagdoll = true
    NilBody()
end)
...
local AutoDropToggle = CashSector:AddToggle("Auto Drop", false, function(State)
    SkidWareSettings.Blatant.Cash.AutoDrop = State
end)

AutoDropToggle:AddSlider(1000, 5000, 10000, 1, function(Value)
    SkidWareSettings.Blatant.Cash.AutoDropAmount = Value
end)

CashSector:AddToggle("Auto Pick Cash", false, function(State)
    SkidWareSettings.Blatant.Cash.AutoPickCash = State
end)
...
local TeleportModule = {
    Food = SkidWareModule.Teleport.Food[1],
    Gun = SkidWareModule.Teleport.Gun[1],
    Location = SkidWareModule.Teleport.Location[1],
    Armor = SkidWareModule.Teleport.Armor[1],
    Melee = SkidWareModule.Teleport.Melee[1],
    Extra = SkidWareModule.Teleport.Extra[1],
    Bike = SkidWareModule.Teleport.Bike[1],
    Mask = SkidWareModule.Teleport.Mask[1],
    Phone = SkidWareModule.Teleport.Phone[1],
    Weight = SkidWareModule.Teleport.Weight[1],
    AutoSet = {
        Tools = {}
    }
}
...
FoodSector:AddDropdown("Food Selection", SkidWareModule.Teleport.Food, SkidWareModule.Teleport.Food[1], false, function(Value)
...
GunSector:AddDropdown("Gun Selection", SkidWareModule.Teleport.Gun, SkidWareModule.Teleport.Gun[1], false, function(Value)
...
GunSector:AddButton("Teleport", function()
    ...
    spawn(function()
        for i = 1, SkidWareSettings.Teleport.AmmoPurchaseAmount do
...
LocationSector:AddDropdown("Location Selection", SkidWareModule.Teleport.Location, SkidWareModule.Teleport.Location[1], false, function(Value)
...
ArmorSector:AddDropdown("Armor Selection", SkidWareModule.Teleport.Armor, SkidWareModule.Teleport.Armor[1], false, function(Value)
...
MeleeSector:AddDropdown("Melee Selection", SkidWareModule.Teleport.Melee, SkidWareModule.Teleport.Melee[1], false, function(Value)
...
ExtraSector:AddDropdown("Extra Selection", SkidWareModule.Teleport.Extra, SkidWareModule.Teleport.Extra[1], false, function(Value)
...
BikeSector:AddDropdown("Bike Selection", SkidWareModule.Teleport.Bike, SkidWareModule.Teleport.Bike[1], false, function(Value)
...
MaskSector:AddDropdown("Mask Selection", SkidWareModule.Teleport.Mask, SkidWareModule.Teleport.Mask[1], false, function(Value)
...
PhoneSector:AddDropdown("Phone Selection", SkidWareModule.Teleport.Phone, SkidWareModule.Teleport.Phone[1], false, function(Value)
...
WeightSector:AddDropdown("Weight Selection", SkidWareModule.Teleport.Weight, SkidWareModule.Teleport.Weight[1], false, function(Value)
...
for i, v in pairs(SkidWareModule.Teleport.Gun) do
    AutoSetTab:AddToggle(v, false, function(State)
        if State then
            if not table.find(TeleportModule.AutoSet.Tools, v) then
                table.insert(TeleportModule.AutoSet.Tools, v)
    ...
    if Alive(LocalPlayer) then
        for i, v in pairs(TeleportModule.AutoSet.Tools) do
            if string.find(v:lower(), "armor") then
                if SkidWareSettings.Teleport.ArmorValueCheck and LocalPlayer.Character.BodyEffects.Armor.Value < 0 then
...
LocalPlayer.CharacterAdded:Connect(function()
    wait(0.5)
    SkidWareModule.God.GodMeleeAlive = false
    if LocalPlayer.Character:FindFirstChild("BodyEffects") then
        if SkidWareModule.God.GodBullet then
            GodFunction(SkidWareModule.God.GodBullet)
            LocalPlayer.Character.BodyEffects.BreakingParts:Destroy()
        end
        if SkidWareModule.God.GodMelee then
            GodFunction(SkidWareModule.God.GodMelee)
            SkidWareModule.God.GodMeleeAlive = true
            LocalPlayer.Character.BodyEffects.Armor:Destroy()
            LocalPlayer.Character.BodyEffects.Defense:Destroy()
        end
        if SkidWareModule.God.AntiRagdoll then
            GodFunction(SkidWareModule.God.AntiRagdoll)
        end
    end
    wait(0.5)
    if SkidWareSettings.Blatant.BlatantAA.Underground then
        Underground(true)
    end
    wait(0.4)
    if SkidWareSettings.Blatant.BlatantAA.UndergroundWallbang then
        Float = Instance.new("BodyVelocity")
        Float.Parent = LocalPlayer.Character.HumanoidRootPart
        Float.MaxForce = Vector3.new(100000, 100000, 100000)
        Float.Velocity = Vector3.new(0, 0, 0)
...
for _, v in next, Players:GetPlayers() do
    if v ~= LocalPlayer and v:IsFriendsWith(LocalPlayer.UserId) then
        Insert(SkidWareSettings.Aiming.Whitelist.Friends, v.Name)
    end
end

Players.PlayerAdded:Connect(function(_Player)
    if _Player ~= LocalPlayer and _Player:IsFriendsWith(LocalPlayer.UserId) then
        Insert(SkidWareSettings.Aiming.Whitelist.Friends, _Player.Name)
    end
end)

Players.PlayerRemoving:Connect(function(_Player)
    if _Player ~= LocalPlayer and _Player:IsFriendsWith(LocalPlayer.UserId) then
        Remove(SkidWareSettings.Aiming.Whitelist.Friends, _Player.Name)
    end
end)
    ...
    for i, v in next, Players:GetPlayers() do
        if v ~= LocalPlayer and Alive(LocalPlayer) and Alive(v) then
            ...
            if Distance > DistanceFromPlayer then
                if (not SkidWareSettings.Aiming.Whitelist.FriendsWhitelist or not table.find(SkidWareSettings.Aiming.Whitelist.Friends, v.Name)) and (not SkidWareSettings.Aiming.Whitelist.CrewEnabled or v:FindFirstChild("DataFolder") and v.DataFolder.Information:FindFirstChild("Crew") and not tonumber(v.DataFolder.Information.Crew.Value) == tonumber(LocalPlayer.DataFolder.Information.Crew.Value)) and (not SkidWareSettings.Aiming.Whitelist.Enabled or not table.find(SkidWareSettings.Aiming.Whitelist.Players, v.Name)) then
                    Target = v
...
function TeleportBuy(Target, AutoSetDelay)
    if workspace.Ignored.Shop:FindFirstChild(Target) and Alive(LocalPlayer) then
        SkidWareModule.Old.CFrame = LocalPlayer.Character.HumanoidRootPart.CFrame
        wait(0.05)
        LocalPlayer.Character.HumanoidRootPart.CFrame = Workspace.Ignored.Shop[Target].Head.CFrame * CFrame.new(0, 3, 0)
        wait(0.15)
        if SkidWareSettings.Teleport.AutoPurchase then
            for i = 1, 10 do
                fireclickdetector(Workspace.Ignored.Shop[Target].ClickDetector)
            end
        end
        if SkidWareSettings.Teleport.TeleportReturn then
            wait(SkidWareSettings.Teleport.ReturnDelay)
            LocalPlayer.Character.HumanoidRootPart.CFrame = SkidWareModule.Old.CFrame  
        end
    ...
    if Alive(LocalPlayer) then
        ...
        for i, v in pairs(Parts) do
            if v.Transparency >= 0.3 then
                SkidWareModule.Instance[#SkidWareModule.Instance + 1] = v
            end
...
function Invisible()
    if Alive(LocalPlayer) then
        SkidWareModule.Old.CFrame = LocalPlayer.Character.HumanoidRootPart.CFrame
        print(SkidWareModule.Old.CFrame)
        wait(0.1)
        ...
        for _, v in pairs(LocalPlayer.Character:GetChildren()) do
            if v:IsA("MeshPart") and not table.find(SkidWareModule.Ignores, v.Name) then
                v:Destroy()
            end
        end
        wait(0.1)
...
UserInputService.InputBegan:Connect(function(Key, Event)
    if Key.UserInputType == Enum.UserInputType.MouseButton2 and not Event then
        SkidWareSettings.Aiming.Aimbot.IsAiming = true
    end
end)

UserInputService.InputEnded:Connect(function(Key, Event)
    if Key.UserInputType == Enum.UserInputType.MouseButton2 and not Event then
        SkidWareSettings.Aiming.Aimbot.IsAiming = false
    end
end)

RunService.Heartbeat:Connect(function()
    if Alive(LocalPlayer) then
        if SkidWareSettings.Blatant.Movement.SpeedEnabled and SkidWareSettings.Blatant.Movement.SpeedType == "CFrame" then
            if SkidWareSettings.Blatant.Movement.SpeedRenderType == "Default" then
                if LocalPlayer.Character.Humanoid.MoveDirection.Magnitude > 0 then
                    for i = 1, SkidWareSettings.Blatant.Movement.SpeedAmount do
                        LocalPlayer.Character:TranslateBy(LocalPlayer.Character.Humanoid.MoveDirection)
                    end
                end
            end
        end
        if SkidWareSettings.Blatant.Character.AntiStomp then
            if Knocked(LocalPlayer) then
                if SkidWareSettings.Blatant.Character.AntiStompType == "Nil Char" then
                    NilBody()
                end
                if SkidWareSettings.Blatant.Character.AntiStompType == "Show Body" then
                    pcall(function()
        ...
        if SkidWareSettings.Blatant.Character.AntiBag then
            if LocalPlayer.Character:FindFirstChild("Christmas_Sock") then
                LocalPlayer.Character:FindFirstChild("Christmas_Sock"):Destroy()
            end
        end
        if SkidWareSettings.Blatant.Character.AntiGrab and LocalPlayer.Character:FindFirstChild("GRABBING_CONSTRAINT") then
...
spawn(function()
    while wait() do
        if Alive(LocalPlayer) then
            if SkidWareSettings.Blatant.Character.AutoLettuce then
                pcall(function()
                    Buy("[Lettuce] - $5", 1)
            ...
            if SkidWareSettings.Blatant.Character.AutoReload then
            ...
            if SkidWareSettings.Blatant.Character.AutoArmor then
                if LocalPlayer.Character.BodyEffects.Armor.Value == 0 then
                    Buy("[High-Medium Armor] - $2300", 0.5, true)
                end
            end
            if SkidWareSettings.Blatant.Farming.MuscleFarm then
                pcall(function()
                    if SkidWareSettings.Blatant.Farming.MuscleFarmingType == "Normal" then
                    ...
                    if SkidWareSettings.Blatant.Farming.MuscleFarmingType == "Heavy" then
            ...
            if SkidWareSettings.Blatant.Farming.ShoeFarm then
                pcall(function()
            ...
            if SkidWareSettings.Blatant.Farming.HospitalFarm then
            ...
            if SkidWareSettings.Blatant.Farming.BoxFarm then
                pcall(function()
...
spawn(function()
    while wait() do
        if SkidWareSettings.Blatant.Farming.ATMFarm then
            for _, v in pairs(Workspace.Cashiers:GetChildren()) do
                if v:FindFirstChild("Head") and v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 then
                    repeat
                        SkidWareSettings.Blatant.Farming.ATMPick = false
                        wait()
                        if SkidWareSettings.Blatant.Farming.ATMFarm then
                            if Alive(LocalPlayer) then
                    ...
                    until v.Humanoid.Health <= 0 or not SkidWareSettings.Blatant.Farming.ATMFarm
                    pcall(function()
                        LocalPlayer.Character:FindFirstChildOfClass("Tool").Parent = LocalPlayer.Backpack
                    end)
                    SkidWareSettings.Blatant.Farming.ATMPick = true
                    wait(5)
                end
            end
        end
...
RunService.RenderStepped:Connect(function()
    if Alive(LocalPlayer) then
        if SkidWareSettings.Aiming.AimingSettings.PingBasedPrediction then
            ...
            if PingNumber < 130 then
                SkidWareSettings.Aiming.AimingSettings.GetVelocity = PingNumber / 1000 + 0.037
            else
                SkidWareSettings.Aiming.AimingSettings.GetVelocity = PingNumber / 1000 + 0.033
            end
        else
            SkidWareSettings.Aiming.AimingSettings.GetVelocity = 0.165
        end
        if SkidWareSettings.Blatant.Cash.AutoDrop then
            ReplicatedStorage.MainEvent:FireServer("DropMoney", tostring(SkidWareSettings.Blatant.Cash.AutoDropAmount))
        end
        if SkidWareSettings.Blatant.Cash.AutoPickCash then
            pcall(function()
        ...
        if SkidWareSettings.Blatant.Farming.ATMPick then
            pcall(function()
                for _, v in pairs(Workspace.Ignored.Drop:GetChildren()) do
                    if v.Name == "MoneyDrop" then
                        local MoneyMagnitude = (v.Position - LocalPlayer.Character.HumanoidRootPart.Position).Magnitude
        ...
        if SkidWareSettings.Blatant.Movement.SpeedEnabled and SkidWareSettings.Blatant.Movement.SpeedType == "CFrame" then
            if SkidWareSettings.Blatant.Movement.SpeedRenderType == "Fast" and Alive(LocalPlayer) then
                if LocalPlayer.Character.Humanoid.MoveDirection.Magnitude > 0 then
                    for i = 1, SkidWareSettings.Blatant.Movement.SpeedAmount do
                        LocalPlayer.Character:TranslateBy(LocalPlayer.Character.Humanoid.MoveDirection)
                    end
        ...
        if SkidWareSettings.Blatant.Reaching.FistReach and LocalPlayer.Character.LeftHand.Transparency ~= 1 then
        ...
        if SkidWareSettings.Blatant.Reaching.MeleeReach then
            local Tool = LocalPlayer.Character:FindFirstChildOfClass("Tool")
            if Tool ~= nil and not Tool:FindFirstChild("Ammo") and TableLowerFind(SkidWareModule.Teleport.Melee, Tool.Name) ~= nil and Tool:FindFirstChild("Handle") then
        ...
        if SkidWareSettings.Blatant.LegitAA.Enabled then
            if SkidWareSettings.Blatant.LegitAA.AntiPointAt then
                for i, v in next, Players:GetPlayers() do
                    if v ~= LocalPlayer and Alive(v) and Alive(LocalPlayer) then
                        ...
                        if BodyEffects ~= nil and MousePos ~= nil then
                            ...
                            if SkidWareSettings.Blatant.LegitAA.AntiAimAtDistance > EnemyMouseMagnitude then
        ...
        if SkidWareSettings.Blatant.BlatantAA.Enabled then
                if SkidWareSettings.Blatant.BlatantAA.AntiAimType == "Jitter" then
                    Jitter(SkidWareSettings.Blatant.BlatantAA.AntiAimSpeed, SkidWareSettings.Blatant.BlatantAA.JitterAngle)
                else
                    Spin(SkidWareSettings.Blatant.BlatantAA.AntiAimSpeed)
                end
                if SkidWareSettings.Blatant.BlatantAA.NoAutoRotate then
    ...
    if SkidWareSettings.Aiming.TargetAimSettings.UnlockTargetKnocked then
        if SkidWareSettings.Aiming.TargetAimSettings.Target ~= nil and Players:FindFirstChild(SkidWareSettings.Aiming.TargetAimSettings.Target) then
            if Knocked(Players:FindFirstChild(SkidWareSettings.Aiming.TargetAimSettings.Target)) then
                SkidWareSettings.Aiming.TargetAimSettings.Target = nil
            end
        end
    end
    if SkidWareSettings.Aiming.SilentAim.ShowFOV then
        SilentAimFOV.Visible = true
        SilentAimFOV.Radius = SkidWareSettings.Aiming.FOV.SilentAimSize
        SilentAimFOV.Filled = SkidWareSettings.Aiming.FOV.FOVFilled
        SilentAimFOV.Transparency = SkidWareSettings.Aiming.FOV.Transparency
        SilentAimFOV.NumSides = 100
        SilentAimFOV.Color = SkidWareSettings.Aiming.SilentAim.FOVColor
        SilentAimFOV.Position = Vector2.new(Mouse.X, Mouse.Y + GuiInset.Y)
    else
        SilentAimFOV.Visible = false
    end
    if SkidWareSettings.Aiming.Aimbot.Enabled then
        if SkidWareSettings.Aiming.Aimbot.ShowFOV then
            AimbotFOV.Visible = true
            AimbotFOV.Radius = SkidWareSettings.Aiming.FOV.AimAssistSize
            AimbotFOV.Filled = SkidWareSettings.Aiming.FOV.FOVFilled
            AimbotFOV.Transparency = SkidWareSettings.Aiming.FOV.Transparency
            AimbotFOV.NumSides = 100
            AimbotFOV.Color = SkidWareSettings.Aiming.Aimbot.FOVColor
            AimbotFOV.Position = Vector2.new(Mouse.X, Mouse.Y + GuiInset.Y)
        else
            AimbotFOV.Visible = false
        end
        if SkidWareSettings.Aiming.Aimbot.AimAssist and SkidWareSettings.Aiming.Aimbot.IsAiming then
            local NearestTarget, NearestDistance = NearestType(SkidWareSettings.Aiming.Aimbot.UseNearestDistance and "Distance" or "Mouse")

            if NearestTarget and (not SkidWareSettings.Aiming.Aimbot.GrabbedCheck or not Grabbing(NearestTarget)) and (not SkidWareSettings.Aiming.Aimbot.KnockedOutCheck or not Knocked(NearestTarget)) and (not SkidWareSettings.Aiming.Aimbot.ShowFOV or SkidWareSettings.Aiming.FOV.AimAssistSize > NearestDistance) and (not SkidWareSettings.Aiming.Aimbot.WallCheck or Visible(NearestTarget.Character.HumanoidRootPart, LocalPlayer.Character.HumanoidRootPart)) then
                local TargetPart = (NearestTarget.Character.Humanoid:GetState() == Enum.HumanoidStateType.Freefall and NearestTarget.Character.LeftFoot or NearestTarget.Character[RandomTable(SkidWareSettings.Aiming.Aimbot.Hitboxes)])
                local Prediction = (SkidWareSettings.Aiming.TargetAimSettings.MovementPrediction and TargetPart.CFrame + (TargetPart.Velocity * SkidWareSettings.Aiming.TargetAimSettings.MovementPredictionAmount) or TargetPart.CFrame + (TargetPart.Velocity * SkidWareSettings.Aiming.AimingSettings.GetVelocity))
                
                if SkidWareSettings.Aiming.AimbotSettings.AimAssistType == "Mouse" then
                    ...
                    local EndPosition = (SkidWareSettings.Aiming.AimbotSettings.SmoothingTracing and Vector2.new((NearestPosition.X - MouseLocation.X) / SkidWareSettings.Aiming.AimbotSettings.SmoothingTracingAmount, (NearestPosition.Y - MouseLocation.Y) / SkidWareSettings.Aiming.AimbotSettings.SmoothingTracingAmount) or Vector2.new((NearestPosition.X - MouseLocation.X) / 1.4, (NearestPosition.Y - MouseLocation.Y) / 1.4))
                    
                    if NearestVisible then
                        mousemoverel(EndPosition.X, EndPosition.Y)
                    end
                elseif SkidWareSettings.Aiming.AimbotSettings.AimAssistType == "Camera" then
...
spawn(function()
    while wait(1 / 16) do
        LagTick = math.clamp(SkidWareModule.LagTick + 1, 0, SkidWareSettings.Blatant.FakeLag.TriggerAmount)
        if Alive(Player) and SkidWareSettings.Blatant.FakeLag.Enabled then
            if LagTick == math.random(1, SkidWareSettings.Blatant.FakeLag.TriggerAmount) then
                Network:SetOutgoingKBPSLimit(9e9)
                SkidWareFolder:ClearAllChildren()
                LagTick = 0
                --[[
                ...
                if SkidWareSettings.Blatant.FakeLag.Enabled then
...
__index = hookmetamethod(game, "__index", function(self, key)
    if self == Mouse and (tostring(key) == "Hit" or tostring(key) == "Target") then
        if SkidWareSettings.Aiming.TargetAim.Enabled then
            if SkidWareSettings.Aiming.TargetAim.Target ~= nil and ChanceTable(SkidWareSettings.Aiming.AimingSettings.HitChanceAmount) == "HitPercent" then
                if Players:FindFirstChild(SkidWareSettings.Aiming.TargetAim.Target) ~= nil and (not SkidWareSettings.Aiming.TargetAim.GrabbedCheck or not Grabbing(Players[SkidWareSettings.Aiming.TargetAim.Target])) and (not SkidWareSettings.Aiming.TargetAim.KnockedOutCheck or not Knocked(Players[SkidWareSettings.Aiming.TargetAim.Target])) and (not SkidWareSettings.Aiming.TargetAim.ShowFOV or SkidWareSettings.Aiming.FOV.TargetAimSize > (LocalPlayer.Character.Head.Position - Players[SkidWareSettings.Aiming.TargetAim.Target].Character.Head.Position).Magnitude) and (not SkidWareSettings.Aiming.TargetAim.WallCheck or Visible(Players[SkidWareSettings.Aiming.TargetAim.Target].Character.HumanoidRootPart, LocalPlayer.Character.HumanoidRootPart)) then
                    local TargetPart = (Players[SkidWareSettings.Aiming.TargetAim.Target].Character.Humanoid:GetState() == Enum.HumanoidStateType.Freefall and Players[SkidWareSettings.Aiming.TargetAim.Target].Character.LeftFoot or Players[SkidWareSettings.Aiming.TargetAim.Target].Character[RandomTable(SkidWareSettings.Aiming.TargetAim.Hitboxes)])
                    local Prediction = (SkidWareSettings.Aiming.TargetAimSettings.MovementPrediction and TargetPart.CFrame + (TargetPart.Velocity * SkidWareSettings.Aiming.TargetAimSettings.MovementPredictionAmount) or TargetPart.CFrame + (TargetPart.Velocity * SkidWareSettings.Aiming.AimingSettings.GetVelocity))
            ...
            if SkidWareSettings.Aiming.SilentAim.Enabled and ChanceTable(SkidWareSettings.Aiming.AimingSettings.HitChanceAmount) == "HitPercent" then
                local NearestTarget, NearestDistance = NearestType(SkidWareSettings.Aiming.SilentAim.UseNearestDistance and "Distance" or "Mouse")
    
                if NearestTarget and (not SkidWareSettings.Aiming.SilentAim.GrabbedCheck or not Grabbing(NearestTarget)) and (not SkidWareSettings.Aiming.SilentAim.KnockedOutCheck or not Knocked(NearestTarget)) and (not SkidWareSettings.Aiming.SilentAim.ShowFOV or SkidWareSettings.Aiming.FOV.SilentAimSize > NearestDistance) and (not SkidWareSettings.Aiming.SilentAim.WallCheck or IsVisible(NearestTarget.Character.Head.Position, {NearestTarget.Character, LocalPlayer.Character, CurrentCamera}) == true) then
                    local TargetPart = (NearestTarget.Character.Humanoid:GetState() == Enum.HumanoidStateType.Freefall and NearestTarget.Character.LeftFoot or NearestTarget.Character[RandomTable(SkidWareSettings.Aiming.SilentAim.Hitboxes)])
                    local Prediction = (SkidWareSettings.Aiming.TargetAimSettings.MovementPrediction and TargetPart.CFrame + (TargetPart.Velocity * SkidWareSettings.Aiming.TargetAimSettings.MovementPredictionAmount) or TargetPart.CFrame + (TargetPart.Velocity * SkidWareSettings.Aiming.AimingSettings.GetVelocity))
...
while wait() do
    if SkidWareSettings.Aiming.TriggerBot.Enabled then
        for i, v in next, Players:GetPlayers() do 
            if Alive(v) then 
                if Mouse.Target:IsDescendantOf(v.Character) then 
                    ...
                    wait()
                    mouse1release()
                    if SkidWareSettings.Aiming.TriggerBot.Delay then
                        wait(SkidWareSettings.Aiming.TriggerBot.DelayAmount)
                    end
                end 
            end
        end
